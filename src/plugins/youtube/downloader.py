...
class YouTubeDownloader(BaseDownloader):
    ...
    def download(self, url: str, save_path: Optional[Path] = None) -> bool:
        ...
        try:
            ...
            if save_path is None:
                ...
                filename = f"{info['title']}_{info['id']}.{self.config.merge_output_format}"
    ...
    def _get_ydl_opts(self, save_path: Path, resume_size: int = 0) -> Dict[str, Any]:
        ...
        if self.config.prefer_quality:
            format_selector.append(f"bestvideo[height<={self.config.prefer_quality}]")
        elif self.config.max_height:
            format_selector.append(f"bestvideo[height<={self.config.max_height}]")
        format_selector.append("bestaudio")
